...
export class Location {
  ...
  private validateName(name: string): void {
    ...
    if (name.length > 100) {
      throw new Error('El nombre de la ubicación no puede exceder 100 caracteres');
    }
  }
  ...
  private validateCode(code: string): void {
    if (!code || code.trim().length < 2) {
      throw new Error('El código de la ubicación debe tener al menos 2 caracteres');
    }
    if (code.length > 20) {
      throw new Error('El código de la ubicación no puede exceder 20 caracteres');
    }
  }
  private validateType(type: string): void {
    if (!type || type.trim().length < 2) {
      throw new Error('El tipo de la ubicación debe tener al menos 2 caracteres');
    }
    if (type.length > 50) {
  ...
  private validateParentId(parentId?: number): void {
    if (parentId !== undefined && parentId <= 0) {
      throw new Error('El ID de la ubicación padre debe ser un número positivo');
    }
  }
