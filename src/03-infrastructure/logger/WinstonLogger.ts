...
import { LoggerWrapperInterface, LogLevel } from '../../02-application/interface/LoggerWrapperInterface';
...
export class WinstonLogger implements LoggerWrapperInterface {
   ...
   * @param metadata - Metadatos adicionales
   */
  async audit(action: string, entityType: string, entityId: number, userId?: number, metadata?: Record<string, any>): Promise<void> {
   ...
   * @param metadata - Metadatos adicionales
   */
  async security(event: string, userId?: number, ipAddress?: string, metadata?: Record<string, any>): Promise<void> {
