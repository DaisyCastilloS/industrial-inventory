...
export interface LoggerWrapperInterface {
   ...
   * @param metadata - Metadatos adicionales
   */
  audit(action: string, entityType: string, entityId: number, userId?: number, metadata?: Record<string, any>): Promise<void>;
   ...
   * @param metadata - Metadatos adicionales
   */
  security(event: string, userId?: number, ipAddress?: string, metadata?: Record<string, any>): Promise<void>;
}